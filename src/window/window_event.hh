...
namespace Parrot {
  ...
  struct WindowCloseRequest {
    /// @trivial
    friend ostream& operator<<(ostream& stream, const WindowCloseRequest& e);
  };

  enum class KeyCode {
    KEY_GER_ESZETT = 1000, KEY_GER_UMLAUT_U = 1001,
    KEY_GER_UMLAUT_O = 1002, KEY_GER_UMLAUT_A = 1003,
    ...
    KEY_KP_DIVIDE = 331, KEY_KP_MULTIPLY = 332,
  ...
  struct MousePress {
    ...
    /// @trivial 
    friend ostream& operator<<(ostream& stream, const MousePress& e);
  ...
  struct MouseMove {
    /// @trivial 
    friend ostream& operator<<(ostream& stream, const MouseMove& e);

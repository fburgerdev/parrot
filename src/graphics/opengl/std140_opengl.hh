...
namespace Parrot {
	...
	constexpr usize BASE_ALIGN_STD140<Array<T, N>> = roundToBaseAlign(BASE_ALIGN_STD140<T>, 16);
	template<class T, usize N, usize M>
	constexpr usize BASE_ALIGN_STD140<Mat<T, N, M>> = roundToBaseAlign(BASE_ALIGN_STD140<T> * N, 16);
	template<class... Types>
	constexpr usize BASE_ALIGN_STD140<Tuple<Types...>> = roundToBaseAlign(max(BASE_ALIGN_STD140<Types>...), 16);
	// SIZE_STD140
	template<>
	...
	template<class T, usize N>
	constexpr usize SIZE_STD140<Array<T, N>> = roundToBaseAlign(SIZE_STD140<T>, 16) * N;
	template<class T, usize N, usize M>
	constexpr usize SIZE_STD140<Mat<T, N, M>> = roundToBaseAlign(SIZE_STD140<T> * N, 16) * M;
	template<class... Types>
	...
	// setSTD140
	inline void setSTD140(bool x, List<uchar>::iterator it) {
		*it = (x ? uchar(1) : uchar(0));
	}
	inline void setSTD140(int32 x, List<uchar>::iterator it) {
		memcpy(&(*it), &x, 4);
	}
	inline void setSTD140(uint32 x, List<uchar>::iterator it) {
		memcpy(&(*it), &x, 4);
	}
	inline void setSTD140(float32 x, List<uchar>::iterator it) {
		memcpy(&(*it), &x, 4);
	}
	inline void setSTD140(float64 x, List<uchar>::iterator it) {
		memcpy(&(*it), &x, 8);
	}
	// :: vec
	template<class T>
	...
	void setSTD140(const Array<T, N>& arr, List<uchar>::iterator it) {
		for (usize i = 0; i < N; ++i) {
			setSTD140(arr.at(i), it + roundToBaseAlign(SIZE_STD140<T>, 16) * i);
		}
	}
	// :: mat
	/*
